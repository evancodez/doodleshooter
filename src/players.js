...
export function encodeLocal(P, weaponIndex, extra = {}) {
  ...
  const out = [+b.pos.x.toFixed(2), +b.pos.y.toFixed(2), +b.pos.z.toFixed(2), +P.yaw.toFixed(2), +P.pitch.toFixed(2), weaponIndex,
    (P.crouching ? 1 : 0) | (P.sliding ? 2 : 0) | (P.isBlocking ? 4 : 0) | (P._aiming ? 8 : 0) | (b.onGround ? 16 : 0) | (extra.firing ? 32 : 0) | (P.alive ? 64 : 0) | (grappling ? 128 : 0) | (P.parryWindow ? 256 : 0),
...
export class RemotePlayer {
  constructor(ctx, id, name, team, ink) {
    ...
    this.isLocal = false; this.alive = true; this.parryWindow = false; this.hp = 100; this.maxHp = 100; this.speed = 0; this.weaponIndex = 0;
  ...
  push(snap, t) {
    ...
    this.grappling = !!(f & 128) && snap.length > 13; if (this.grappling) this.gPoint.set(snap[11], snap[12], snap[13]); this.parryWindow = !!(f & 256);
